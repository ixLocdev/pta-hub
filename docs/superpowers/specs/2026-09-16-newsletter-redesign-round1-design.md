...
| announcement | — | `button_url` | `sanitize_link_url` (new helper, Decision 6) | new |
...
| quick_notes | — | `label`, `items[]` `{heading,body,link_url,link_text}` | `sanitize_text_field`, `sanitize_text_field` / `wp_kses_post` / `sanitize_link_url` / `sanitize_text_field` | new type |
